...
<body>
    ...
    <div class="course">
        ...
        <span id="ProfessorName">{{ $professor->name }}</span>
    </div>

    @foreach($subjects as $subject)

        <div class="card" onclick="document.getElementById('{{ $subject->subject_id }}').submit()">
            <form action="{{ route('show.prof.attendance') }}" method="GET" id="{{ $subject->subject_id }}">
                
                <!-- Hidden input for subject ID -->
                <input type="hidden" name="subjectID" value="{{ $subject->subject_id }}">

                <div>
                    <span id="subjectSched">{{ $subject->subject_code }} / {{ $subject->course_year_block }}</span>
                </div>
